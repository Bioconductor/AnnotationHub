...
setReplaceMethod("allVersions", "Hub",
    function(x, value)
{
    stopifnot(value %in% c(TRUE, FALSE))
    if (allVersions(x) != value){
        ...
        x <- new(as.character(class(x)), cache=hubCache(x), hub=hubUrl(x),
                 date=db_date, .db_path=db_path, .db_uid=db_uid,
                 isLocalHub=isLocalHub(x), allVersions=value)
    }
    invisible(x)
})
    ...
    function(x, ..., value)
{
    ...
    # reset hub = important especially if using localHub
    # will not reset original object but will reset subsetting object
    localHub <- isLocalHub(x)
    db_path <- x@.db_path
    if (!localHub){
        db_date <- .restrictDateByVersion(db_path)
...
### - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
### Removing Resources from downloaded cache
###
...
.show1 <- function(object)
{
    ...
    cat("# names(): ", id, "\n", sep="")
    cat("# full name: ", names(object)[[1]], "\n", sep="")
    cat("# version: ", version, "\n", sep="")
    cat("# versions available: ",
        paste0(sub('.*\\.', '', tbl$ah_id), collapse=", "), "\n", sep="")
