...
files (ex. `AnnotationHub(localHub=TRUE)`) and a resource cannot be found in
...
Error: Cannot retrieve resource
  ...
  Requested resource not found in local cache:
    AH66165 : 72911
```
The options are to rerun the constructor with
`AnnotationHub(localHub=FALSE)` so the file can be downloaded. If this cannot be
done because of internet access or other issues, the resource will not be
