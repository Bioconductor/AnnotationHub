...
can take NO arguments as input.  It can rely on other helper
functions, but ultimately it (and it's helpers need to know how to
find resources and how to process those resources into
\Robject{AnnotationHubMetadata} objects on it's own.  
