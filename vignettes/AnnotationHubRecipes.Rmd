...
makeAnnotationHubResource function to do some automatic setup.  This
...
into the NAMESPACE (remember that this is that string you are providing as your 1st argument), and then add this code to the AnnotationHubData repository.  
...
Later when we run your recipe, we will add a call like this (along with a new class name export to the NAMESPACE file) to AnnotationHubData.  But for now you just need to have called this function in order to test your recipe.


## Step 4: Testing your new recipe

At this point you are probably wondering if this recipe will even work.  Well there is a way to test it here.  You can call the function that will try to update the resources for your recipe.  That function is called updateResources(). The updateResources() function takes several arguments.  These are:
...
* insert: try to insert metadata into the DB? As a recipe author, you don't get to do this.  So it should be FALSE when you call this or you will get an error.
