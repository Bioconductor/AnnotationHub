...
- Creating a helper function which returns a data.frame containing
  information about the files that we want to add to AnnotationHub.
- Writing a function which takes the data.frame from above and
...
## Step 2: Function for pre-processing the File (Recipe Field)

The 2nd kind of function you need to write is called a recipe
function.  It always must take an single argument that must be an 
AnnotationHubMetadata object.  The job of a recipe function is to 
use the metadata from an AnnotationHubMetadata object to produce an 
...
1) SourceUrls should merelu document the original location of the untouched file
...
One can post-process the file when it is rendered via AnnotationHub in the user's cache. 
